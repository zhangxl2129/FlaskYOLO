...
class BaseMixTransform:
    ...
    probability-based application of transforms and manages the mixing of multiple imgs and labels.

    Attributes:
        dataset (Any): The dataset object containing imgs and labels.
    ...
    Methods:
        ...
        get_indexes: Abstract method to get indexes of imgs to be mixed.
        _update_label_text: Updates label text for mixed imgs.
    ...
    def __init__(self, dataset, pre_transform=None, p=0.0) -> None:
        ...
        Args:
            dataset (Any): The dataset object containing imgs and labels for mixing.
            pre_transform (Callable | None): Optional transform to apply before mixing.
    ...
    def __call__(self, labels):
        ...
        selects additional imgs, applies pre-transforms if specified, and then performs the mix transform.

        Args:
            labels (Dict): A dictionary containing label data for an image.

        Returns:
            (Dict): The transformed labels dictionary, which may include mixed data from other imgs.
        ...
        # Get index of one or three other imgs
        indexes = self.get_indexes()
        if isinstance(indexes, int):
            indexes = [indexes]

        # Get imgs information will be used for Mosaic or MixUp
...
class Mosaic(BaseMixTransform):
    ...
    This class performs mosaic augmentation by combining multiple (4 or 9) imgs into a single mosaic image.
    ...
    def get_indexes(self, buffer=True):
        ...
        the 'buffer' parameter. It is used to choose imgs for creating mosaic augmentations.

        Args:
            buffer (bool): If True, selects imgs from the dataset buffer. If False, selects from the entire
                dataset.

        Returns:
            (List[int]): A list of random image indexes. The length of the list is n-1, where n is the number
                of imgs used in the mosaic (either 3 or 8, depending on whether n is 4 or 9).
        ...
        """
        if buffer:  # select imgs from buffer
            return random.choices(list(self.dataset.buffer), k=self.n - 1)
        else:  # select any imgs
    ...
    def _mix_transform(self, labels):
        ...
        This method combines multiple imgs (3, 4, or 9) into a single mosaic image based on the 'n' attribute.
        It ensures that rectangular annotations are not present and that there are other imgs available for
        mosaic augmentation.

        Args:
            labels (Dict): A dictionary containing image data and annotations. Expected keys include:
                - 'rect_shape': Should be None as rect and mosaic are mutually exclusive.
                - 'mix_labels': A list of dictionaries containing data for other imgs to be used in the mosaic.
        ...
        assert len(labels.get("mix_labels", [])), "There are no other imgs for mosaic augment."
        return (
    ...
    def _mosaic3(self, labels):
        """
        Creates a 1x3 image mosaic by combining three imgs.

        This method arranges three imgs in a horizontal layout, with the main image in the center and two
        additional imgs on either side. It's part of the Mosaic augmentation technique used in object detection.
        ...
        Args:
            labels (Dict): A dictionary containing image and label information for the main (center) image.
                ...
                dictionaries containing information for the side imgs.
    ...
    def _mosaic4(self, labels):
        """
        Creates a 2x2 image mosaic from four input imgs.

        This method combines four imgs into a single mosaic image by placing them in a 2x2 grid. It also
        ...
        Args:
            labels (Dict): A dictionary containing image data and labels for the base image (index 0) and three
                additional imgs (indices 1-3) in the 'mix_labels' key.

        Returns:
            (Dict): A dictionary containing the mosaic image and updated labels. The 'img' key contains the mosaic
                image as a numpy array, and other keys contain the combined and adjusted labels for all four imgs.
    ...
    def _mosaic9(self, labels):
        """
        Creates a 3x3 image mosaic from the input image and eight additional imgs.

        This method combines nine imgs into a single mosaic image. The input image is placed at the center,
        and eight additional imgs from the dataset are placed around it in a 3x3 grid pattern.
        ...
        Args:
            labels (Dict): A dictionary containing the input image and its associated labels. It should have
                ...
                - 'mix_labels' (List[Dict]): A list of dictionaries containing information for the additional
                  eight imgs, each with the same structure as the input labels.
    ...
    def _cat_labels(self, mosaic_labels):
        ...
        This method combines labels from multiple imgs used in mosaic augmentation, clips instances to the
...
class MixUp(BaseMixTransform):
    ...
    Minimization" (https://arxiv.org/abs/1710.09412). MixUp combines two imgs and their labels using a random weight.
    ...
    def __init__(self, dataset, pre_transform=None, p=0.0) -> None:
        ...
        MixUp is an image augmentation technique that combines two imgs by taking a weighted sum of their pixel
        values and labels. This implementation is designed for use with the Ultralytics YOLO framework.

        Args:
            dataset (Any): The dataset to which MixUp augmentation will be applied.
            pre_transform (Callable | None): Optional transform to apply to imgs before MixUp.
...
class RandomPerspective:
    """
    Implements random perspective and affine transformations on imgs and corresponding annotations.

    This class applies random rotations, translations, scaling, shearing, and perspective transformations
    to imgs and their associated bounding boxes, segments, and keypoints. It can be used as part of an
    ...
    Methods:
        ...
        __call__: Applies the random perspective transformation to imgs and annotations.
    ...
    def __init__(
        ...
        This class implements random perspective and affine transformations on imgs and corresponding bounding boxes,
...
class RandomHSV:
    ...
    This class applies random HSV augmentation to imgs within predefined limits set by hgain, sgain, and vgain.
...
class LetterBox:
    ...
    This class resizes and pads imgs to a specified shape while preserving aspect ratio. It also updates
    ...
    def __init__(self, new_shape=(640, 640), auto=False, scaleFill=False, scaleup=True, center=True, stride=32):
        """
        Initialize LetterBox object for resizing and padding imgs.

        This class is designed to resize and pad imgs for object detection, instance segmentation, and pose estimation
...
class CopyPaste(BaseMixTransform):
    ...
    different imgs to create new training samples.
    ...
    Methods:
        ...
        __call__: Applies the Copy-Paste transformation to imgs and annotations.
...
class Format:
    ...
    Attributes:
        ...
        bgr (float): The probability to return BGR imgs.
    ...
    def __init__(
        ...
        Args:
            ...
            bgr (float): Probability of returning BGR imgs instead of RGB.
...
def v8_transforms(dataset, imgsz, hyp, stretch=False):
    ...
    This function creates a composition of image augmentation techniques to prepare imgs for YOLO training.
    ...
    Examples:
        >>> from ultralytics.data.dataset import YOLODataset
        >>> dataset = YOLODataset(img_path="path/to/imgs", imgsz=640)
...
def classify_transforms(
    ...
    This function generates a sequence of torchvision transforms suitable for preprocessing imgs
...
class ClassifyLetterBox:
    """
    A class for resizing and padding imgs for classification tasks.

    This class is designed to be part of a transformation pipeline, e.g., T.Compose([LetterBox(size), ToTensor()]).
    It resizes and pads imgs to a specified size while maintaining the original aspect ratio.
    ...
    def __init__(self, size=(640, 640), auto=False, stride=32):
        ...
        pads imgs to a specified size while maintaining the original aspect ratio.
...
class CenterCrop:
    """
    Applies center cropping to imgs for classification tasks.

    This class performs center cropping on input imgs, resizing them to a specified size while maintaining the aspect
    ...
    def __init__(self, size=640):
        ...
        It performs a center crop on input imgs to a specified size.
...
class ToTensor:
    ...
    def __init__(self, half=False):
        """
        Initializes the ToTensor object for converting imgs to PyTorch tensors.
