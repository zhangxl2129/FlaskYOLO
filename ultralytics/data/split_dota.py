...
def load_yolo_dota(data_root, split="train"):
    ...
    im_dir = Path(data_root) / "imgs" / split
    assert im_dir.exists(), f"Can't find {im_dir}, please check your data root."
    im_files = glob(str(Path(data_root) / "imgs" / split / "*"))
...
def crop_and_save(anno, windows, window_objs, im_dir, lb_dir, allow_background_images=True):
    """
    Crop imgs and save new labels.

    Args:
        anno (dict): Annotation dict, including `filepath`, `label`, `ori_size` as its keys.
        windows (list): A list of windows coordinates.
        window_objs (list): A list of labels inside each window.
        im_dir (str): The output directory path of imgs.
        lb_dir (str): The output directory path of labels.
        allow_background_images (bool): Whether to include background imgs without labels.
        ...
        and the output directory structure is:
            - save_dir
                ...
                - labels
                    - split
    """
    im_dir = Path(save_dir) / "imgs" / split
...
def split_test(data_root, save_dir, crop_size=1024, gap=200, rates=(1.0,)):
    ...
    for r in rates:
        crop_sizes.append(int(crop_size / r))
        gaps.append(int(gap / r))
    save_dir = Path(save_dir) / "imgs" / "test"
    save_dir.mkdir(parents=True, exist_ok=True)

    im_dir = Path(data_root) / "imgs" / "test"
    assert im_dir.exists(), f"Can't find {im_dir}, please check your data root."
