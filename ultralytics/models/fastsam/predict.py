...
class FastSAMPredictor(SegmentationPredictor):
    ...
    def _clip_inference(self, images, texts):
        ...
        Args:
            images (List[PIL.Image]): A list of source imgs and each of them should be PIL.Image type with RGB channel order.
            texts (List[str]): A list of prompt texts and each of them should be string object.

        Returns:
            (torch.Tensor): The similarity between given imgs and texts.
        """
        try:
            import clip
        except ImportError:
