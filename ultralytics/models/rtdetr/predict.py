...
class RTDETRPredictor(BasePredictor):
    ...
    def postprocess(self, preds, img, orig_imgs):
        ...
        Args:
            ...
            img (torch.Tensor): Processed input imgs.
            orig_imgs (list or torch.Tensor): Original, unprocessed imgs.
    ...
    def pre_transform(self, im):
        """
        Pre-transforms the input imgs before feeding them into the model for inference. The input imgs are
        letterboxed to ensure a square aspect ratio and scale-filled. The size must be square(640) and scaleFilled.

        Args:
            im (list[np.ndarray] |torch.Tensor): Input imgs of shape (N,3,h,w) for tensor, [(h,w,3) x N] for list.

        Returns:
            (list): List of pre-transformed imgs ready for model inference.
        """
