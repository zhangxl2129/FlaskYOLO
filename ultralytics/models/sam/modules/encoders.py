...
class ImageEncoderViT(nn.Module):
    """
    An image encoder using Vision Transformer (ViT) architecture for encoding imgs into a compact latent space.

    This class processes imgs by splitting them into patches, applying transformer blocks, and generating a final
    encoded representation through a neck module.

    Attributes:
        img_size (int): Dimension of input imgs, assumed to be square.
    ...
    def __init__(
        ...
        """
        Initializes an ImageEncoderViT instance for encoding imgs using Vision Transformer architecture.
        ...
        Attributes:
            img_size (int): Dimension of input imgs.
            patch_embed (PatchEmbed): Module for patch embedding.
            pos_embed (nn.Parameter | None): Absolute positional embedding for patches.
...
class ImageEncoder(nn.Module):
    """
    Encodes imgs using a trunk-neck architecture, producing multiscale features and positional encodings.
...
class Hiera(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> List[torch.Tensor]:
        """Performs forward pass through Hiera model, extracting multiscale features from input imgs."""
        x = self.patch_embed(x)
        # x: (B, H, W, C)
