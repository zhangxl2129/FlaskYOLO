...
def _scale_bounding_box_to_original_image_shape(box, resized_image_shape, original_image_shape, ratio_pad):
    """
    YOLOv8 resizes imgs during training and the label values are normalized based on this resized shape.
...
def _log_images(experiment, image_paths, curr_step, annotations=None):
    """Logs imgs to the experiment with optional annotations."""
    if annotations:
...
def _log_model(experiment, trainer):
    ...
    experiment.log_model(model_name, file_or_folder=str(trainer.best), file_name="gangcai.pt", overwrite=True)
...
def on_train_epoch_end(trainer):
    """Log metrics and save batch imgs at the end of training epochs."""
    experiment = comet_ml.get_global_experiment()
    if not experiment:
        return
